 ...
 */
@Component
public class CopyOfAlarmMessageHandler extends IoHandlerAdapter
{
		...
		if (null != device)    // 如果存在该控制器
		{
			...
			if (status.equals(""))   // 控制器正常
			{
				if (errorDevices.containsKey(address))
				{
					...
					sendRecoverShortMessage(sysMessage, device, pointInfo);
				}
			} 
			else    // 控制器状态不正常
			{
	...
	private void haddleProber(ExplorerMessage explorerMessage, PointInfo pointInfo, String timeLaber)
	{
		...
		if (null != device) // 如果存在该设备
		{
			...
			if (status.equals("") && errorDevices.containsKey(address))   // 状态正常同时此前有故障或报警
			{
				...
				sendRecoverShortMessage(explorerMessage, device, pointInfo);
			}
			else // 不正常
			{
