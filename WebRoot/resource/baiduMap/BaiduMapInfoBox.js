...
(function() {
    //声明baidu包
    var T, baidu = T = baidu || {version: '2.0'};
    baidu.guid = '$BAIDU$';
     ...
     * @constructor
         ...
         * }<br />.
         * @example <b>参考示例：</b><br />
         * var infoBox = new BMapLib.InfoBox(map,"百度地图api",{boxStyle:{background:"url('resource/baiduMap/img/tipbox.gif') no-repeat
     ...
     */
        ...
        this._opts.closeIconUrl = opts.closeIconUrl || "resource/baiduMap/img/close.png";
    ...
    baidu.object.extend(InfoBox.prototype,{
        ...
        _adjustPosition: function(poi){
            ...
            switch(this._opts.align){
                case INFOBOX_AT_TOP:
                    if(this._marker){
                        this._div.style.bottom = -(pixel.y - this._opts.offset.height - icon.anchor.height) + 2 + "px";
                    }else{
            ...
            if(this._marker){
                this._div.style.left = pixel.x - icon.anchor.width - this._boxWidth / 2 + "px";
            }else{
        ...
        _panBox: function(){
            ...
            switch(this._opts.align){
                case INFOBOX_AT_TOP:
                    //上侧超出
                    var h = this._marker ? icon.anchor.height + this._marker.getOffset().height : 0;
